...
const Gallery = () => {
  ...
  <div>
    <div className="w-9/12 max-w-xl mx-auto">
        <h1 className="font-extrabold text-center text-6xl py-4 font-menu">Our Gallery</h1>
    </div>
      {fullScreenImageUrl && (
    ...
    <div className="w-9/12 border-t-2 border-black mx-auto">
      <div className="container mx-auto my-10">
        <div>
          <h2 className="pb-3 text-3xl font-bold"><u>Food & Drinks</u></h2>
        </div>
        <div className="grid md:grid-cols-3 gap-4">
          {foodPics.map((item, index) => (
            <div className="rounded-sm overflow-hidden w-82 h-80 pb-full flex items-center" key={index}>
              <img 
              // className=""
              src={item.src}
              alt={item.alt}
        ...
        <div>
          <h2 className="pt-6 pb-3 text-3xl font-bold"><u>Restaurant</u></h2>
        </div>
        <div className="grid md:grid-cols-3 gap-4">
          {restaurantPics.map((item, index) => (
            <div className="bg-gray-300 rounded-sm overflow-hidden w-82 h-80 pb-full flex items-center" key={index}>
              <img 
